...
#This version allows for application rate (alpha) to change through time via forcing function
...
dSaltForce <- function(time, state, pars) {
  with(as.list(c(state,pars)), {
    #Rate of change in Cl- in watershed, kg y-1
    dSW <- alpha(time)*delta*A - p*phi*SW
