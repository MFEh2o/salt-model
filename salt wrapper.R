...
#Note tat this is meters of road per area but NOT lane-m of road per area.
...
ggplot() +
  ...
  scale_fill_manual(values = rev(met.brewer("Peru1", 6)), name=expression(atop(Road~salt,'('*mg~Cl^'-'~L^-1*')'))) +
  guides(fill=guide_legend(override.aes=list(size=3))) +
  # scale_fill_gradientn(colors = rev(met.brewer("Peru1"))) +
  # scale_color_gradientn(colors = rev(met.brewer("Peru1"))) +
  theme_bw(base_size = 9)

ggsave('figures/saltLakes.jpg', height = 4, width = 6, dpi = 300)  
